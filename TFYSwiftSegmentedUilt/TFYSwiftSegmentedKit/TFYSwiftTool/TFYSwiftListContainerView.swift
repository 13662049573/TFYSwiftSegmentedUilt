//
//  TFYSwiftListContainerView.swift
...
public enum TFYSwiftListContainerType {
    case scrollView
    case collectionView
}

public protocol TFYSwiftViewListContainer {
...
@objc
public protocol TFYSwiftListContainerViewListDelegate {
    /// 如果列表是VC，就返回VC.view
    /// 如果列表是View，就返回View自己
    ///
    /// - Returns: 返回列表视图
...
@objc
public protocol TFYSwiftListContainerViewDataSource {
    /// 返回list的数量
    func numberOfLists(in listContainerView: TFYSwiftListContainerView) -> Int

    /// 根据index初始化一个对应列表实例，需要是遵从`TFYSwiftListContainerViewListDelegate`协议的对象。
    /// 如果列表是用自定义UIView封装的，就让自定义UIView遵从`TFYSwiftListContainerViewListDelegate`协议，该方法返回自定义UIView即可。
    /// 如果列表是用自定义UIViewController封装的，就让自定义UIViewController遵从`TFYSwiftListContainerViewListDelegate`协议，该方法返回自定义UIViewController即可。
    /// 注意：一定要是新生成的实例！！！
    ///
    /// - Parameters:
    ///   - listContainerView: TFYSwiftListContainerView
    ///   - index: 目标index
    /// - Returns: 遵从TFYSwiftListContainerViewListDelegate协议的实例
    ...
    @objc optional func listContainerView(_ listContainerView: TFYSwiftListContainerView, canInitListAt index: Int) -> Bool

    /// 返回自定义UIScrollView或UICollectionView的Class
    /// 某些特殊情况需要自己处理UIScrollView内部逻辑。比如项目用了FDFullscreenPopGesture，需要处理手势相关代理。
    ///
    /// - Parameter listContainerView: TFYSwiftListContainerView
...
open class TFYSwiftListContainerView: UIView, TFYSwiftViewListContainer, TFYSwiftViewRTLCompatible {
    ...
    open private(set) var validListDict = [Int:TFYSwiftListContainerViewListDelegate]()
    ...
    public init(dataSource: TFYSwiftListContainerViewDataSource, type: TFYSwiftListContainerType = .scrollView) {
...
class TFYSwiftListContainerViewController: UIViewController {

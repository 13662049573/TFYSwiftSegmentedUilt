//
//  TFYSwiftIndicatorDotLineView.swift
...
open class TFYSwiftIndicatorDotLineView: TFYSwiftIndicatorBaseView {
    /// 线的最大宽度
    ...
    open override func contentScrollViewDidScroll(model: TFYSwiftIndicatorTransitionParams) {
        ...
        if percent <= 0.5 {
            targetX = TFYSwiftViewTool.interpolate(from: leftX, to: centerX, percent: CGFloat(percent*2))
            targetWidth = TFYSwiftViewTool.interpolate(from: dotWidth, to: lineMaxWidth, percent: CGFloat(percent*2))
        }else {
            targetX = TFYSwiftViewTool.interpolate(from: centerX, to: rightX, percent: CGFloat((percent - 0.5)*2))
            targetWidth = TFYSwiftViewTool.interpolate(from: lineMaxWidth, to: dotWidth, percent: CGFloat((percent - 0.5)*2))
        }

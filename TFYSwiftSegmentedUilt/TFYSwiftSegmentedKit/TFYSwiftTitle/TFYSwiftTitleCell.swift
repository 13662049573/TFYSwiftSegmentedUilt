...
open class TFYSwiftTitleCell: TFYSwiftBaseCell {
    ...
    open func preferredTitleZoomAnimateClosure(itemModel: TFYSwiftTitleItemModel, baseScale: CGFloat) -> TFYSwiftCellSelectedAnimationClosure {
        return {[weak self] (percnet) in
            if itemModel.isSelected {
                //将要选中，scale从小到大插值渐变
                itemModel.titleCurrentZoomScale = TFYSwiftViewTool.interpolate(from: itemModel.titleNormalZoomScale, to: itemModel.titleSelectedZoomScale, percent: percnet)
            }else {
                //将要取消选中，scale从大到小插值渐变
                itemModel.titleCurrentZoomScale = TFYSwiftViewTool.interpolate(from: itemModel.titleSelectedZoomScale, to:itemModel.titleNormalZoomScale , percent: percnet)
            }
    ...
    open func preferredTitleStrokeWidthAnimateClosure(itemModel: TFYSwiftTitleItemModel, attriText: NSMutableAttributedString) -> TFYSwiftCellSelectedAnimationClosure{
        return {[weak self] (percent) in
            if itemModel.isSelected {
                //将要选中，StrokeWidth从小到大插值渐变
                itemModel.titleCurrentStrokeWidth = TFYSwiftViewTool.interpolate(from: itemModel.titleNormalStrokeWidth, to: itemModel.titleSelectedStrokeWidth, percent: percent)
            }else {
    ...
    open func preferredTitleColorAnimateClosure(itemModel: TFYSwiftTitleItemModel) -> TFYSwiftCellSelectedAnimationClosure {
        return {[weak self] (percent) in
            if itemModel.isSelected {
                //将要选中，textColor从titleNormalColor到titleSelectedColor插值渐变
                itemModel.titleCurrentColor = TFYSwiftViewTool.interpolateThemeColor(from: itemModel.titleNormalColor, to: itemModel.titleSelectedColor, percent: percent)
            }else {
                //将要取消选中，textColor从titleSelectedColor到titleNormalColor插值渐变
                itemModel.titleCurrentColor = TFYSwiftViewTool.interpolateThemeColor(from: itemModel.titleSelectedColor, to: itemModel.titleNormalColor, percent: percent)
            }
    ...
    override func setSelectedStyle(isSelected: Bool) {
        if isSelected {
            self.titleLabel.textColor = (self.itemModel as? TFYSwiftTitleItemModel)?.titleSelectedColor
        } else {
            self.titleLabel.textColor = (self.itemModel as? TFYSwiftTitleItemModel)?.titleNormalColor
        }
    }
}

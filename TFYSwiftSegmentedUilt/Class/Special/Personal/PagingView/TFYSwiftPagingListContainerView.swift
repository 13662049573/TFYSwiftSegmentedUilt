//
//  TFYSwiftPagingListContainerView.swift
//  TFYSwiftView
//
...
public enum TFYSwiftPagingListContainerType {
    case scrollView
    case collectionView
}

@objc
public protocol TFYSwiftPagingViewListViewDelegate {
    /// 如果列表是VC，就返回VC.view
    /// 如果列表是View，就返回View自己
    ///
    /// - Returns: 返回列表视图
...
@objc
public protocol TFYSwiftPagingListContainerViewDataSource {
    /// 返回list的数量
    ///
    /// - Parameter listContainerView: TFYSwiftPagingListContainerView
    func numberOfLists(in listContainerView: TFYSwiftPagingListContainerView) -> Int

    /// 根据index初始化一个对应列表实例，需要是遵从`TFYSwiftPagingViewListViewDelegate`协议的对象。
    /// 如果列表是用自定义UIView封装的，就让自定义UIView遵从`TFYSwiftPagingViewListViewDelegate`协议，该方法返回自定义UIView即可。
    /// 如果列表是用自定义UIViewController封装的，就让自定义UIViewController遵从`TFYSwiftPagingViewListViewDelegate`协议，该方法返回自定义UIViewController即可。
    /// 注意：一定要是新生成的实例！！！
    ///
    /// - Parameters:
    ///   - listContainerView: TFYSwiftPagingListContainerView
    ///   - index: 目标index
    /// - Returns: 遵从TFYSwiftPagingViewListViewDelegate协议的实例
    func listContainerView(_ listContainerView: TFYSwiftPagingListContainerView, initListAt index: Int) -> TFYSwiftPagingViewListViewDelegate


    /// 控制能否初始化对应index的列表。有些业务需求，需要在某些情况才允许初始化某些列表，通过通过该代理实现控制。
    @objc optional func listContainerView(_ listContainerView: TFYSwiftPagingListContainerView, canInitListAt index: Int) -> Bool
    ...
    @objc optional func scrollViewClass(in listContainerView: TFYSwiftPagingListContainerView) -> AnyClass
}

@objc protocol TFYSwiftPagingListContainerViewDelegate {
    @objc optional func listContainerViewDidScroll(_ listContainerView: TFYSwiftPagingListContainerView)
    @objc optional func listContainerViewWillBeginDragging(_ listContainerView: TFYSwiftPagingListContainerView)
    @objc optional func listContainerViewDidEndScrolling(_ listContainerView: TFYSwiftPagingListContainerView)
...
open class TFYSwiftPagingListContainerView: UIView {
    ...
    public var isCategoryNestPagingEnabled = false {
        didSet {
            if let containerScrollView = scrollView as? TFYSwiftPagingListContainerScrollView {
                containerScrollView.isCategoryNestPagingEnabled = isCategoryNestPagingEnabled
            }else if let containerScrollView = scrollView as? TFYSwiftPagingListContainerCollectionView {
                containerScrollView.isCategoryNestPagingEnabled = isCategoryNestPagingEnabled
            }
        }
    }
    /// 已经加载过的列表字典。key是index，value是对应的列表
    open var validListDict = [Int:TFYSwiftPagingViewListViewDelegate]()
    ...
    weak var delegate: TFYSwiftPagingListContainerViewDelegate?
    private var currentIndex: Int = 0
    private var collectionView: UICollectionView!
    private var containerVC: TFYSwiftPagingListContainerViewController!
    private var willAppearIndex: Int = -1
    private var willDisappearIndex: Int = -1

    public init(dataSource: TFYSwiftPagingListContainerViewDataSource, type: TFYSwiftPagingListContainerType = .collectionView) {
    ...
    open func commonInit() {
        ...
        containerVC = TFYSwiftPagingListContainerViewController()
        ...
        if type == .scrollView {
            if let scrollViewClass = dataSource.scrollViewClass?(in: self) as? UIScrollView.Type {
                ...
                scrollView = TFYSwiftPagingListContainerScrollView.init()
            }
            ...
            if let collectionViewClass = dataSource.scrollViewClass?(in: self) as? UICollectionView.Type {
                ...
                collectionView = TFYSwiftPagingListContainerCollectionView.init(frame: CGRect.zero, collectionViewLayout: layout)
            }
    ...
    //MARK: - TFYSwiftViewListContainer
...
class TFYSwiftPagingListContainerViewController: UIViewController {
